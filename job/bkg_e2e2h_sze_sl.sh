...
k=0
flag=0
n=`ls /cefs/data/DstData/CEPC240/CEPC_v4/4fermions/E240.Psze_sl.e0.p0.whizard195/*slcio -l | grep "^-" | wc -l`
let "n = $n - 1"
for SLCIO in `ls /cefs/data/DstData/CEPC240/CEPC_v4/4fermions/E240.Psze_sl.e0.p0.whizard195/*slcio -l | awk '{print $9}'`
do
	if [ $flag == 0 ]; then
		STEER_FILE="../steer/bkg_e2e2h_sze_sl_"$j".steer"
		JOB_FILE="bkg_e2e2h_sze_sl_"$j
		echo "" > $STEER_FILE
	...
	if [ $i -ge $FILENUM ]; then
		...
		echo "" >> $STEER_FILE
		echo "    TreeOutputFile   ../splitted/bkg_e2e2h_sze_sl_"$j".root" >> $STEER_FILE
		echo ".end -------------------------------------------------" >> $STEER_FILE
