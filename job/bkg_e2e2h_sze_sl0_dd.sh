...
k=0
flag=0
n=`ls /cefs/data/RecData/CEPC250/CEPC_v1/4fermions/E250.Psze_sl.e0.p0.whizard195/dd/*slcio -l | grep "^-" | wc -l`
let "n = $n - 1"
for SLCIO in `ls /cefs/data/RecData/CEPC250/CEPC_v1/4fermions/E250.Psze_sl.e0.p0.whizard195/dd/*slcio -l | awk '{print $9}'`
do
	if [ $flag == 0 ]; then
		STEER_FILE="../steer/bkg_e2e2h_sze_sl0_dd_"$j".steer"
		JOB_FILE="bkg_e2e2h_sze_sl0_dd_"$j
		echo "" > $STEER_FILE
	...
	if [ $i -ge $FILENUM ]; then
		...
		echo "" >> $STEER_FILE
		echo "    TreeOutputFile   ../splitted/bkg_e2e2h_sze_sl0_dd_"$j".root" >> $STEER_FILE
		echo ".end -------------------------------------------------" >> $STEER_FILE
