...
k=0
flag=0
n=`ls /cefs/dirac/user/b/byzhang/ww_h/cuxx_01/rec/*slcio -l | grep "^-" | wc -l`
let "n = $n - 1"
for SLCIO in `ls /cefs/dirac/user/b/byzhang/ww_h/cuxx_01/rec/*slcio -l | awk '{print $9}'`
do
	if [ $flag == 0 ]; then
		STEER_FILE="../steer/bkg_e2e2h_ww_h_cuxx_"$j".steer"
		JOB_FILE="bkg_e2e2h_ww_h_cuxx_"$j
		echo "" > $STEER_FILE
	...
	if [ $i -ge $FILENUM ]; then
		...
		echo "" >> $STEER_FILE
		echo "    TreeOutputFile   ../splitted/bkg_e2e2h_ww_h_cuxx_"$j".root" >> $STEER_FILE
		echo ".end -------------------------------------------------" >> $STEER_FILE
