...
k=0
flag=0
n=`ls /cefs/tmp_storage/moxin/rec_samples/e1e1h_invi/rec/*slcio -l | grep "^-" | wc -l`
let "n = $n - 1"
for SLCIO in `ls /cefs/tmp_storage/moxin/rec_samples/e1e1h_invi/rec/*slcio -l | awk '{print $9}'`
do
	if [ $flag == 0 ]; then
		STEER_FILE="../steer/signal_e1e1h_"$j".steer"
		JOB_FILE="signal_e1e1h_"$j
		echo "" > $STEER_FILE
