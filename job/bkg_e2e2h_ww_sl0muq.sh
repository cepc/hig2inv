...
k=0
flag=0
n=`ls /cefs/data/RecData/CEPC250/CEPC_v1/4fermions/E250.Pww_sl.e0.p0.whizard195/ww_sl0muq/ww_sl0muq.e0.p0.00141_rec_*slcio -l | grep "^-" | wc -l`
let "n = $n - 1"
for SLCIO in `ls /cefs/data/RecData/CEPC250/CEPC_v1/4fermions/E250.Pww_sl.e0.p0.whizard195/ww_sl0muq/ww_sl0muq.e0.p0.00141_rec_*slcio -l | awk '{print $9}'`
do
	if [ $flag == 0 ]; then
	...
	if [ $i -ge $FILENUM ]; then
		...
		echo "" >> $STEER_FILE
		echo "    TreeOutputFile   ../rawdata/bkg_e2e2h_ww_sl0muq_"$j".root" >> $STEER_FILE
		echo ".end -------------------------------------------------" >> $STEER_FILE
