void fithiggs()
{
   ...
   TString rootfile;
   rootfile = "../sel/sig_bkg_e2e2h.root";
    
   TString  epsname;  
   epsname="../figs/fithiggs.eps";
   ...
   TTree *t = (TTree *)f->Get("MCPart");
   
   Float_t m_m_recoil;
   t->SetBranchAddress("m_m_recoil", &m_m_recoil);
   
   TH1F* histData=new TH1F("Recoil mass of Z boson","Recoil mass of Z boson",100,120,150);
   ...
   RooRealVar mean("mean_CB","mean_CB",125,122,127);
   RooRealVar sigma("sgm_CB","sgm_CB",1,0,5);
   RooRealVar alpha("alpha_CB","alpha_CB",-5,-10,0);
   RooRealVar n("n","n",10,0.0,200.0);
   ...
   RooRealVar mean0("mean0", "mean0", 125, 120, 135);
   RooRealVar sigma0("sigma0", "sigma0",10,0,30);
   ...
   RooRealVar mean1("mean1", "mean1", 126, 120, 130);
   RooRealVar sigma1("sigma1", "sigma1",10,0,30);
   RooGaussian gauss1("gauss1"," gauss fit ",x, mean1, sigma1);

   Double_t mentr = (Double_t)histData->GetEntries();
   Double_t msig = mentr;
   Double_t mbkg = 0.5 * mentr;
